...
#include <cstdlib>
#include <fcntl.h> // Для open(), mkstemp()
#include <fstream>
#include <iostream>
#include <sstream>
#include <string>
#include <sys/stat.h>
#include <sys/types.h> // Дополнительные типы для POSIX API
#include <unistd.h>    // Для close(), unlink(), write()
...
// Returns true on error.
static bool format(StringRef FileName, bool ErrorOnIncompleteFormat = false, StringRef Content = StringRef()) {
  ...
  if (!Content.empty()) {
    // Используем переданные данные вместо чтения из файла/stdin
    Code = MemoryBuffer::getMemBuffer(Content, FileName);
  } else {
    // Стандартная логика чтения из файла или stdin
...
std::string runCustomFormatterAndGetResult(const std::string &content) {
  ...
  char input_file[] = "/tmp/custom_format_input_XXXXXX";
  char output_file[] = "/tmp/custom_format_output_XXXXXX";
  
  int in_fd = mkstemp(input_file);
  int out_fd = mkstemp(output_file);
  
  if (in_fd == -1 || out_fd == -1) {
    llvm::errs() << "Failed to create temporary files\n";
    if (in_fd != -1) {
      close(in_fd);
      unlink(input_file);
    }
    ...
    return content; // Возвращаем исходный контент в случае ошибки
  }
  
  // Записываем содержимое во входной файл
  write(in_fd, content.c_str(), content.size());
  close(in_fd);
  close(out_fd);
  ...
  // Считываем отформатированное содержимое
  std::string formatted_content;
  
  if (result == 0) {
    std::ifstream formattedFile(output_file);
    formatted_content = std::string((std::istreambuf_iterator<char>(formattedFile)),
                                   std::istreambuf_iterator<char>());
  } else {
    // В случае ошибки Python-скрипта возвращаем исходный контент
